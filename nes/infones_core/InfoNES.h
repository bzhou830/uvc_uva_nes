...
/*-------------------------------------------------------------------*/
/*  PSRAM placement helper (BL616)                                   */
/*  The BL616 has only ~414 KB SRAM, but InfoNES needs far more      */
/*  working memory. The large emulator buffers are placed in the 4 MB*/
/*  on-chip PSRAM (region ram_psram @ 0xA8000000) via this macro.    */
/*                                                                   */
/*  We use ".psram_noinit" (NOLOAD) rather than ".psram_bss" because */
/*  the SDK linker template's .psram_bss output section has no input*/
/*  wildcard, so .psram_bss variables become orphans placed in the  */
/*  internal SRAM (overflow). ".psram_noinit" IS collected into the  */
/*  ram_psram region by the linker script. The buffers are explicitly*/
/*  zeroed in nes_bridge_init(), so the missing startup zeroing of  */
/*  the NOLOAD section is irrelevant.                                */
/*  PSRAM is initialised by board_psram_x8_init() when CONFIG_PSRAM  */
/*  is set in defconfig.                                             */
/*-------------------------------------------------------------------*/
